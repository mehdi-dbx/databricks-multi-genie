import { a as message, c as and, d as gte, f as inArray, i as chat, l as eq, m as sql, n as isDatabaseAvailable, o as asc, p as lt, r as drizzle, s as desc, u as gt } from "./connection-pool-BJJY6K-P.mjs";
import "./src-BaHhVWSg.mjs";
import { n as getAuthMethodDescription, r as getAuthSession, t as getAuthMethod } from "./src-B0YMvyB0.mjs";
import { t as src_default$1 } from "./src-CdTKJz_q.mjs";
import { n as CONTEXT_HEADER_USER_ID, t as CONTEXT_HEADER_CONVERSATION_ID } from "./src-CaSFj_Sa.mjs";
...
if (!process.env.TEST_MODE) {
	...
	dotenv.config({ path: path.resolve(__dirname$1, "../..", ".env.local") });
	dotenv.config({ path: path.resolve(__dirname$1, "../../..", ".env.local") });
}
...
async function ensureDb() {
	...
	if (authMethod === "oauth" || authMethod === "cli") {
		...
		try {
			const { getDb } = await import("./connection-pool-CqIX9-Ry.mjs");
...
async function getServerProvider() {
	const { getDatabricksServerProvider } = await import("./src-BAi0Lx8-.mjs");
...
chatRouter.post("/", requireAuth, async (req, res) => {
	...
	try {
		...
		if (!chat$1) {
			if (isDatabaseAvailable() && message$1) {
		...
		const previousMessages = !dbAvailable || !message$1 && requestBody.previousMessages ? requestBody.previousMessages ?? [] : convertToUIMessages(messagesFromDb);
		let uiMessages;
		if (message$1) {
			uiMessages = [...previousMessages, message$1];
			...
			if (dbAvailable && requestBody.previousMessages) {
				...
				if (assistantMessages.length > 0) {
					...
					if (requestBody.previousMessages?.some((m) => m.parts?.some((p) => p.type === "dynamic-tool" && (p.state === "output-denied" || "approval" in p && p.approval?.approved === false)))) {
						res.end();
						return;
					}
				}
...
*/
tablesRouter.get("/:tableName", requireAuth, async (req, res) => {
...
app.post("/api/reset-state", authMiddleware, requireAuth, (req, res) => {
	...
	const proc = spawn("uv", [
		"run",
		"python",
		path.join(projectRoot, "scripts", "reset_state.py")
	], {
		cwd: projectRoot,
		stdio: [
			"ignore",
...
const taskEventClients = [];
app.get("/api/events/tasks", (req, res) => {
	const assignedTo = typeof req.query.assigned_to === "string" ? req.query.assigned_to : void 0;
	res.setHeader("Content-Type", "text/event-stream");
	res.setHeader("Cache-Control", "no-cache");
	res.setHeader("Connection", "keep-alive");
	res.flushHeaders?.();
	taskEventClients.push({
		res,
		assignedTo
	});
	console.log(`[task-events] SSE client connected assigned_to=${assignedTo ?? "any"} total=${taskEventClients.length}`);
...
app.post("/api/events/task-created", (req, res) => {
	...
	if (!assignedToId || typeof assignedToId !== "string") return res.status(400).json({ error: "assigned_to_id required" });
...
app.post("/api/audio/transcribe", upload.single("file"), async (req, res) => {
	...
	formData.append("file", new Blob([file.buffer], { type: file.mimetype || "audio/webm" }), file.originalname || "recording.webm");
...
if (isDevelopment) app.get("/", (_req, res) => {
...
async function startServer() {
	if (process.env.PLAYWRIGHT === "True") {
		...
		try {
			...
			const { getCapturedRequests, resetCapturedRequests, getLastCapturedRequest } = await import(path.join(dirname(dirname(__dirname)), "tests", "api-mocking", "api-mock-handlers.ts"));
			...
			app.get("/api/test/last-captured-request", (_req, res) => {
				...
				if (lastRequest) res.json(lastRequest);
				else res.status(404).json({ error: "No captured requests" });
			});
